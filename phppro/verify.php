...
if(isset($_REQUEST['token'])) {
    ...
    if(!empty($data)) {
        if($QueryFire->upDateTable("users",' access_token="'.trim(strip_tags($_REQUEST['token'])).'"', array('access_token'=>'','is_verified'=>1))) {
            ...
            // Additional headers
            $headers .= 'From:Admin <info@sakshijewels.com>' . "\r\n";
              $template = file_get_contents('mail_template.php');
              $template = str_replace('%name%', $data['name'] , $template);
              $template = str_replace('%data%', "You have verified your account on <a href=". base_url .">". base_url ."</a><br><br> For any help kindly contact us at info@sakshijewels.com" , $template);
              $template = str_replace('%link2%', base_url , $template);
              $template = str_replace('%link2text%', 'Sakshijewels' , $template);
