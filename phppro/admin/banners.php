...
if(isset($_POST['addHomebanner'])) {
  if(isset($_FILES) && !empty($_FILES)) {
    $ret = $QueryFire->fileUpload($_FILES['file_upload'],'../images/banners/');
    if($ret['status'] && isset($ret['image_name'])) {
      ...
      if($QueryFire->insertData('banners',$arr)) {
        $msg = 'banner added successfully';
      } else {
        $msg = "Unable to add banner";
      }
    } else {
...
if(isset($_POST['updateHomebanner'])) {
  ...
  if($ret['status'] && isset($ret['image_name'])) {
    $arr['image_name'] = $ret['image_name'];
    $duplicate = $QueryFire->getAllData('banners',' id ='.$_REQUEST['id']);
    unlinkImage('../images/banners/'.$duplicate[0]['image_name']);
  } else {
    $msg = 'Unable to upload file. Please try later.';
  }
  if($QueryFire->upDateTable("banners",'id ='.$_REQUEST['id'],$arr)) {
    $msg = 'banner successfully updated.';
  } else {
    $msg = 'Can not update banner.';
  }
}
...
if(!empty($_POST['user_action'])) {
  switch($_POST['user_action']) {
    case 'delete':
        $duplicate = $QueryFire->getAllData('banners',' id ='.$_REQUEST['id']);
        unlinkImage('../images/banners/'.$duplicate[0]['image_name']);
        $QueryFire->deleteDataFromTable("banners",' id='.$_POST['id']);
        $msg = 'banner deleted successfully.';
        break;
  }
}
$banners = $QueryFire->getAllData('banners',' 1');
?>
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>Banners <button class="btn btn-primary float-right" data-toggle="modal" data-target="#myModal">Add Banner</button></h1>
        </div>
        <div class="col-sm-6">
            ...
            <table class="data-table table table-bordered table-striped table-bordered table-hover">
              ...
              <tbody>
                <?php if(!empty($banners)) {
                  ...
                  <tr>
                    <td class="cat">
                      <img src="../images/banners/<?php echo $row['image_name'];?>" class="img-responsive img-thumbnail">
                    </td>
                    ...
                    <td>
                      <button class="btn btn-success btn-xs edit_banner mt-1" data-id="<?php echo $row['id'];?>">Edit</button>
                      <a class="btn btn-danger btn-xs" data-toggle="modal" data-target="#deleteModal" data-id="<?php echo $row['id'];?>"> Delete</a> 
                    </td>
                  </tr>
                <?php } } else {
                  echo '<td colspan="7" class="text-center">You have not added any banner</td>';
                } ?>
              </tbody>
            </table>
          </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="deleteModalLabel">Are you sure you want to delete this Banner</h5>
    ...
    <form id="addGal" method="post" action="" enctype="multipart/form-data">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary" name="addHomebanner" >Add</button>
  ...
  <!-- Edit Modal -->
  <div id="editHomebanner" class="modal fade" role="dialog">
    <form method="post" action="" id="editGalImg" enctype="multipart/form-data">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary" name="updateHomebanner" >Update</button>
            <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
          </div>
        </div>
      </div>
